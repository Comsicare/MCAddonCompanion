...
export default {
  ...
  setup() {
    ...
    const preflightModal = ref(null)

    const openPreflight = async (instName, moveOnly) => {
      preflightModal.value = { phase: 'scanning', instName, moveOnly }
    ...
    const closePreflight = () => { preflightModal.value = null }

    const startArchiveFromPreflight = () => {
      const pf = preflightModal.value
      ...
      doArchive(pf.instName, pf.moveOnly, dhExclusions)
    }

    const doArchive = async (instName, moveOnly, dhExclusions) => {
      ...
      try {
        ...
        const r = await window.pywebview.api[method](instName, dhExclusions || null)
        if (!r.ok && !r.started) {
          window.__onProgress = prevHandler
    ...
    return {
      ...
      preflightModal, openPreflight, closePreflight, startArchiveFromPreflight,
              ...
              <div class="danger-zone">
                ...
                <div style="margin-bottom:8px">
                  <template v-if="archiveConfirm === 'archive'">
                    ...
                    <div class="flex items-center gap-6">
                      <button class="btn-danger" @click="openPreflight(settingsModal.instance, false)">Confirm Archive</button>
                ...
                <div>
                  <template v-if="archiveConfirm === 'move_only'">
                    ...
                    <div class="flex items-center gap-6">
                      <button class="btn-danger" @click="openPreflight(settingsModal.instance, true)">Yes, move only</button>
                      <button class="btn btn-ghost btn-sm" @click="archiveConfirm = null">Cancel</button>
                    </div>
                  </template>
                  <template v-else>
      ...
      <div v-if="preflightModal" style="position:fixed;inset:0;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center;z-index:10000">
        <div style="background:var(--bg-1);border:1px solid var(--line);border-radius:12px;width:500px;max-width:95vw;max-height:85vh;display:flex;flex-direction:column;overflow:hidden">
          <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 20px;border-bottom:1px solid var(--line);flex:none">
            ...
            <button class="icon-btn" @click="closePreflight"><span v-html="icon('x',14)"></span></button>
          </div>
            ...
            <template v-else>
              <div style="display:flex;gap:20px">
                <div>
                  ...
                  <div class="mono fs-14">{{ preflightModal.folderSizeMb }} MB</div>
                </div>
                <div>
                  <div class="kicker" style="margin-bottom:4px">Files</div>
                  <div class="mono fs-14">{{ preflightModal.fileCount }}</div>
                </div>
              </div>
              <template v-if="preflightModal.dhEntries.length">
                <div>
                  <div class="kicker" style="margin-bottom:8px">DistantHorizons LOD Data</div>
                  <div style="display:flex;flex-direction:column;gap:6px">
                    <div style="display:grid;gap:8px;padding:4px 0;border-bottom:1px solid var(--line)" :style="preflightModal.moveOnly ? 'grid-template-columns:1fr auto' : 'grid-template-columns:1fr auto auto'">
                      <span class="fs-11 text-3">Entry</span>
                      <span class="fs-11 text-3" style="text-align:center">Excl. Sync</span>
                      <span v-if="!preflightModal.moveOnly" class="fs-11 text-3" style="text-align:center">Excl. Backup</span>
                    </div>
                    <template v-for="entry in preflightModal.dhEntries" :key="entry.path_rel">
                      <div style="display:grid;gap:8px;align-items:center;padding:4px 0" :style="preflightModal.moveOnly ? 'grid-template-columns:1fr auto' : 'grid-template-columns:1fr auto auto'">
                        <div>
                          <div class="fs-13 text-0">{{ entry.type === 'server' ? 'Server' : 'World' }}: {{ entry.name }}</div>
                          <div class="fs-11 text-3">{{ entry.size_mb }} MB</div>
                        </div>
                        <input type="checkbox" v-model="preflightModal.exclusions[entry.path_rel].sync" style="cursor:pointer">
                        <input v-if="!preflightModal.moveOnly" type="checkbox" v-model="preflightModal.exclusions[entry.path_rel].zip" style="cursor:pointer">
                      </div>
                    </template>
                  </div>
                </div>
                <template v-for="entry in preflightModal.dhEntries" :key="'warn-' + entry.path_rel">
                  <div v-if="preflightModal.moveOnly && preflightModal.exclusions[entry.path_rel].sync"
                  ...
                  <div v-if="!preflightModal.moveOnly && preflightModal.exclusions[entry.path_rel].sync && preflightModal.exclusions[entry.path_rel].zip"
                    style="padding:8px 12px;background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.35);border-radius:6px;font-size:12px;color:#f59e0b">
